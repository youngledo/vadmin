...
public final class CustomersView extends SecuredView {
    ...
    public CustomersView(SecurityContextCurrentUserProvider currentUser, AuthorizationService authorization,
        ...
        create.setVisible(authorization.hasPermission(requireCurrentUser(), PermissionCode.of("customer:customer:create")));
        ...
        add(new H1("客户"), new HorizontalLayout(filter, create), grid);
    }

    @Override PermissionCode requiredPermission() { return PermissionCode.of("customer:customer:read"); }
    ...
    private void edit(Customer customer) {
        ...
        if (customer != null) { name.setValue(customer.name()); email.setValue(customer.email()); active.setValue(customer.active()); }
        else { active.setValue(true); }
        var save = new Button("保存", event -> {
            if (customer == null) customers.create(requireCurrentUser(), name.getValue(), email.getValue());
            else customers.update(requireCurrentUser(), customer.id(), name.getValue(), email.getValue(), active.getValue());
            dialog.close(); pages.refresh();
        });
        dialog.add(new VerticalLayout(name, email, active, new HorizontalLayout(save, new Button("取消", event -> dialog.close()))));
        dialog.open();
    }
    ...
    private void showAttachments(Customer customer, AuthorizationService authorization) {
        ...
        var upload = new Upload(com.vaadin.flow.server.streams.UploadHandler.inMemory((metadata, content) -> {
            var stored = fileUpload.store(requireCurrentUser(), PermissionCode.of("customer:attachment:upload"),
        ...
        upload.setVisible(authorization.hasPermission(requireCurrentUser(), PermissionCode.of("customer:attachment:upload")));
